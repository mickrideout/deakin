...
def produce_word_cloud(regex_word_array, df: pd.DataFrame):
    ...
    # print a table of the top 10 words and their frequency
    print(pd.DataFrame(list(word_freq.items()), columns=['Word', 'Frequency']).nlargest(10, 'Frequency'))
...
## Word cloud of python libraries

```{python}
# Python datascience libraries from chatgpt
...
def get_user_location(row):
    ...
    print(f"Searching: {location_text}, Found: {found_location}")
...
def geocode(row):
    ...
    try:
        ...
        found_location = geolocator.geocode(location)
        if found_location:
            # Get the country from the location
            found_location = location.address.split(",")[-1].strip()
            print(f"Searching: {location_text}, Found: {found_location}")
            return found_location
...
users_df.apply(geocode, axis=1)

```

## Most frequent external url links
...
def extract_domains(urls):
    try:
        if isinstance(urls, str):
            urls = [urls]
        return [urlparse(url).netloc for url in urls if isinstance(url, str)]
    except Exception as e:
        return []



# From posts_df['Body'] extract all urls
url_pattern = r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
posts_df['urls'] = posts_df['Body'].str.findall(url_pattern)
# for all the urls, extract the domain
# remove any floats from post_df['urls']

posts_df['domains'] = posts_df['urls'].apply(extract_domains)
posts_df['domains'].value_counts()

# flatten domain arrays to strings
posts_df['domains'] = posts_df['domains'].apply(lambda x: ' '.join(x))

# flatten all the domains
domains_text = posts_df['domains'].str.cat(sep=' ')

# regex to remove i.stack.imgur.com domain as this is just an image site
domains_text = re.sub(r'i.stack.imgur.com', '', domains_text)

# produce a domain frequency map
    # Create a word frequency dictionary
domain_freq = {}
...
# create a barplot of the top 10 domains
domain_freq_df = pd.DataFrame(list(domain_freq.items()), columns=['Domain', 'Frequency']).nlargest(10, 'Frequency')
domain_freq_df.plot.bar(x='Domain', y='Frequency', figsize=(10, 5))
plt.title('Top 10 Most Frequently Referenced Domains')
plt.ylabel('Reference Frequency')
plt.xlabel('Domain')
plt.show()
